...
- [kotlin 协程中 Launch vs Async 的比较](https://amitshekhar.me/blog/launch-vs-async-in-kotlin-coroutines)
- [kotlin 协程中的程序调度者 Dispatchers](https://amitshekhar.me/blog/dispatchers-in-kotlin-coroutines) 
...
- [Long-running tasks in parallel with Kotlin Flow](https://amitshekhar.me/blog/long-running-tasks-in-parallel-with-kotlin-flow) - 长时间运行的任务与 Kotlin Flow 并行
- [Retry Operator in Kotlin Flow](https://amitshekhar.me/blog/retry-operator-in-kotlin-flow) - Kotlin Flow 中的重试运算符
- [Callback to Coroutines in Kotlin](https://amitshekhar.me/blog/callback-to-coroutines-in-kotlin) - 回调到 Kotlin 中的协程
...
* **为什么 Android 应用程序会滞后？** - [Learn from here](https://amitshekhar.me/blog/android-app-lag)

* **什么是Context？它是如何使用的？- Android 应用程序中的上下文** - [Context In Android Application](https://amitshekhar.me/blog/context-in-android-application)

* **告诉所有的 Android 应用程序组件。-从这里学习** - [Learn from here](https://developer.android.com/guide/components/fundamentals.html#Components)

* **Android 应用程序的项目结构是什么？-从这里学习** - [Learn from here](https://developer.android.com/studio/projects)

* **是什么AndroidManifest.xml？**
...
#### Activity and Fragment 活动和片段

* **为什么建议只使用默认构造函数来创建一个Fragment？-从这里学习** - [Learn from here](https://www.youtube.com/watch?v=CitBt0FZFIc)

* **What is `Activity` and its lifecycle?什么是Activity它的生命周期？**

* **onCreate() 和 onStart() 有什么区别 What is the difference between onCreate() and onStart()**

* **当没有 onPause() 和 onStop() 的活动只调用 onDestroy 时？-从这里学习 When only onDestroy is called for an activity without onPause() and onStop()?** - [Learn from here](https://www.youtube.com/watch?v=B2kY_ckZa-g)

* **为什么我们需要在Activity类的onCreate()中调用setContentView()？-从这里学习Why do we need to call setContentView() in onCreate() of Activity class?** - [Learn from here](https://www.youtube.com/watch?v=U1aHAt7XC5I)

* **活动中的 onSavedInstanceState() 和 onRestoreInstanceState() 是什么？What is onSavedInstanceState() and onRestoreInstanceState() in activity?**
    - onSavedInstanceState() - This method is used to store data before pausing the activity.
    - onSavedInstanceState() - 此方法用于在暂停活动之前存储数据。
    - onRestoreInstanceState() - This method is used to recover the saved state of an activity when the activity is recreated after destruction. So, the onRestoreInstanceState() receive the bundle that contains the instance state information.
    - onRestoreInstanceState() - 此方法用于在销毁后重新创建活动时恢复活动的已保存状态。因此，onRestoreInstanceState() 接收包含实例状态信息的包。

* **什么是Fragment它的生命周期。What is `Fragment` and its lifecycle.**

* **什么是“启动模式”？What are "launchMode"?** - [Learn from here](https://amitshekhar.me/blog/singletask-launchmode-in-android) and [singleTask launchMode in Android](https://youtu.be/WYkQEnm4jeI)

* **Fragmenta和 an 和有什么不一样Activity？说明两者的关系。 What is the difference between a `Fragment` and an `Activity`? Explain the relationship between the two.** - [Learn from here](https://stackoverflow.com/questions/10478233/why-fragments-and-when-to-use-fragments-instead-of-activities)

* **When should you use a Fragment rather than an Activity?**
    - When you have some UI components to be used across various activities
    - 什么时候应该使用 Fragment 而不是 Activity？
    - When multiple view can be displayed side by side just like viewPager
    - 当您有一些 UI 组件要在各种活动中使用时
...
* **FragmentPagerAdapter 和 FragmentStatePagerAdapter 有什么区别？What is the difference between FragmentPagerAdapter vs FragmentStatePagerAdapter?**
    ...
    - FragmentPagerAdapter：用户访问的每个片段都会存储在内存中，但视图会被销毁。重新访问页面时，将创建视图而不是片段的实例。
    - FragmentStatePagerAdapter: Here, the fragment instance will be destroyed when it is not visible to the user, except the saved state of the fragment.
    - FragmentStatePagerAdapter：这里的fragment实例在用户不可见时会被销毁，fragment的保存状态除外。

* **在 backstack 中添加/替换片段有什么区别？-从这里学习 What is the difference between adding/replacing fragment in backstack?** - [Learn from here](https://stackoverflow.com/questions/24466302/basic-difference-between-add-and-replace-method-of-fragment/24466345)

* **你将如何在两个 Fragment 之间进行通信？How would you communicate between two Fragments?**

* **什么是保留Fragment？What is retained `Fragment`?**
    - By default, Fragments are destroyed and recreated along with their parent Activity’s when a configuration change occurs. Calling setRetainInstance(true) allows us to bypass this destroy-and-recreate cycle, signaling the system to retain the current instance of the fragment when the activity is recreated.
    - 默认情况下，当发生配置更改时，Fragment 会与其父 Activity 一起被销毁和重新创建。调用 setRetainInstance(true) 允许我们绕过这个销毁和重新创建循环，向系统发出信号以在重新创建活动时保留片段的当前实例。

* **addToBackStack()提交片段事务的目的是什么？What is the purpose of `addToBackStack()` while commiting fragment transaction?**
    ...
    - 通过调用 addToBackStack()，将替换事务保存到后退堆栈，这样用户可以通过按“后退”按钮来撤销事务并返回前一个片段。从这里了解更多

#### Views and ViewGroups 视图和视图组

* **View Android 中有什么？What is `View` in Android?**

* **View.GONE和之间的区别View.INVISIBLE？-从这里学习 Difference between `View.GONE` and `View.INVISIBLE`?** - [Learn from here](https://stackoverflow.com/questions/11556607/android-difference-between-invisible-and-gone)

* **你能创建自定义视图吗？如何？Can you a create custom view? How?**
...
* **什么是 ViewGroup 以及它们与 View 有何不同？What are ViewGroups and how they are different from the Views?**
    ...
    - 视图：视图对象是 Android 中用户界面 (UI) 元素的基本构建块。视图是一个响应用户操作的简单矩形框。例如EditText、Button、CheckBox等。View是指android.view.View类，它是所有UI类的基类。
    - ViewGroup: ViewGroup is the invisible container. It holds View and ViewGroup. For example, LinearLayout is the ViewGroup that contains Button(View), and other Layouts also. ViewGroup is the base class for Layouts.
    - ViewGroup：ViewGroup是不可见的容器。它拥有 View 和 ViewGroup。例如，LinearLayout 是包含 Button(View) 的 ViewGroup，其他 Layouts 也是如此。ViewGroup 是布局的基类。

* **什么是画布？What is a Canvas?**

* **什么是SurfaceView？-从这里学习What is a `SurfaceView`?** - [Learn from here](https://developer.android.com/reference/android/view/SurfaceView)

* **相对布局与线性布局。Relative Layout vs Linear Layout.**

* **讲述约束布局 Tell about Constraint Layout**

* **你知道什么是视图树吗？你如何优化它的深度？-从这里学习Do you know what is the view tree? How can you optimize its depth?** - [Learn from here](https://developer.android.com/reference/android/view/ViewTreeObserver)
...
* **ListView和 和有什么不一样RecyclerView？-从这里学习 What is the difference between `ListView` and `RecyclerView`?** - [Learn from here](https://stackoverflow.com/questions/26728651/recyclerview-vs-listview)

* **RecyclerView 内部是如何工作的？How does RecyclerView work internally?**

* **什么是 ViewHolder 模式？我们为什么要使用它？-从这里学习What is the ViewHolder pattern? Why should we use it?** - [Learn from here](https://stackoverflow.com/questions/21501316/what-is-the-benefit-of-viewholder-pattern-in-android)

* **RecyclerView优化-滚动性能提升-从这里学习 RecyclerView Optimization - Scrolling Performance Improvement** - [Learn from here](https://amitshekhar.me/blog/recyclerview-optimization)

* **优化嵌套 RecyclerView -从这里学习 Optimizing Nested RecyclerView** - [Learn from here](https://amitshekhar.me/blog/setrecycledviewpool-for-optimizing-nested-recyclerview)

* **是什么SnapHelper？- 从这里学习：SnapHelper What is `SnapHelper`?** - Learn from here: [SnapHelper](https://amitshekhar.me/blog/snaphelper)

#### Dialogs and Toasts 对话和吐司

* **DialogAndroid 中有什么？-从这里学习 What is `Dialog` in Android?** - [Learn from here](https://developer.android.com/guide/topics/ui/dialogs)

* **ToastAndroid 中有什么？-从这里学习 What is `Toast` in Android?** - [Learn from here](https://developer.android.com/guide/topics/ui/notifiers/toasts)

* **Dialog和之间有什么区别Dialog Fragment？-从这里学习 What the difference between `Dialog` and `Dialog Fragment`?** - [Learn from here](https://stackoverflow.com/questions/7977392/android-dialogfragment-vs-dialog)

#### Intents and Broadcasting 意图和广播

* **是什么Intent？What is `Intent`?**

* **什么是隐式Intent？What is an Implicit `Intent`?**

* **什么是显式Intent？What is an Explicit `Intent`?**

* **什么是BroadcastReceiver？-从这里学习 What is a `BroadcastReceiver`?** - [Learn from here](https://developer.android.com/guide/components/broadcasts)

* **什么是LocalBroadcastManager？ What is a `LocalBroadcastManager`?**

* **的功能是什么IntentFilter？-从这里学习 What is the function of an `IntentFilter`?** - [Learn from here](https://developer.android.com/reference/android/content/IntentFilter)

* **什么是粘性Intent？What is a Sticky `Intent`?**
    ...
    - Sticky Intents 允许函数和服务之间的通信。sendStickyBroadcast() 执行称为粘性的 sendBroadcast(Intent)，即您发送的 Intent 在广播完成后仍然存在，以便其他人可以通过 registerReceiver(BroadcastReceiver, IntentFilter) 的返回值快速检索该数据。例如，如果您使用 ACTION_BATTERY_CHANGED 的意图来获取电池更换事件：当您为该操作调用 registerReceiver() 时——即使使用 null BroadcastReceiver——您将获得该操作的最后一次广播的 Intent。因此，您可以使用它来查找电池的状态，而不必注册电池中所有未来的状态变化。

* **描述广播和意图如何工作以便能够在您的应用程序周围传递消息？-从这里学习Describe how broadcasts and intents work to be able to pass messages around your app?** - [Learn from here](https://stackoverflow.com/questions/7276537/using-a-broadcast-intent-broadcast-receiver-to-send-messages-from-a-service-to-a)

* **什么是PendingIntent？What is a `PendingIntent`?**
    - If you want someone to perform any Intent operation at future point of time on behalf of you, then we will use Pending Intent.
    - 如果你希望某人在未来某个时间点代表你执行任何 Intent 操作，那么我们将使用 Pending Intent。

* **有哪些不同类型的广播？-从这里学习What are the different types of Broadcasts?** - [Learn from here](https://developer.android.com/guide/components/broadcasts)

#### Services服务

* **是什么Service？-从这里学习What is `Service`?** - [Learn from here](https://developer.android.com/guide/components/services)

* **Service对比IntentService `Service` vs `IntentService`.**

* **什么是JobScheduler？-从这里学习 What is a `JobScheduler`?** - [Learn from here](https://developer.android.com/reference/android/app/job/JobScheduler)

#### Inter-process Communication 进程间通信

* **两个截然不同的 Android 应用程序如何交互？-从这里学习 How can two distinct Android apps interact?** - [Learn from here](https://developer.android.com/training/basics/intents)

* **是否可以在多个进程中运行 Android 应用程序？如何？-从这里学习 Is it possible to run an Android app in multiple processes? How?** - [Learn from here](https://stackoverflow.com/questions/6567768/how-can-an-android-application-have-more-than-one-process)

* **什么是 AIDL？列举通过 AIDL 创建有界服务的步骤。-从这里学习What is AIDL? Enumerate the steps in creating a bounded service through AIDL.** - [Learn from here](https://developer.android.com/guide/components/aidl)

* **您可以在 Android 中使用什么进行后台处理？-从这里学习What can you use for background processing in Android?** - [Learn from here](https://developer.android.com/guide/background)

* **什么是a ContentProvider，它通常用于什么？-从这里和这里学习What is a `ContentProvider` and what is it typically used for?** - [Learn from here](https://developer.android.com/guide/topics/providers/content-provider-basics) and [here](https://developer.android.com/guide/topics/providers/content-providers)

#### Long-running Operations长时间运行的操作

* **如何在 Java 或 Android 中并行运行任务，并在全部完成时获得回调？-与 Kotlin Flow 并行的长时间运行的任务How to run parallel tasks in Java or Android, and get callback when all complete?** - [Long-running tasks in parallel with Kotlin Flow](https://amitshekhar.me/blog/long-running-tasks-in-parallel-with-kotlin-flow)

* **为什么要避免在主线程上运行非 ui 代码？-从这里学习Why should you avoid to run non-ui code on the main thread?** - [Learn from here](https://developer.android.com/training/multiple-threads/communicate-ui)

* **什么是 ANR？如何预防 ANR？-从这里学习What is ANR? How can the ANR be prevented?** - [Learn from here](https://developer.android.com/topic/performance/vitals/anr.html)

* **什么是AsyncTask（在 API 级别 30 中弃用）？What is an `AsyncTask`(Deprecated in API level 30) ?**

* **AsyncTask 有什么问题？What are the problems in AsyncTask?**

* **您什么时候会使用 java 线程而不是 AsyncTask？-从这里学习When would you use java thread instead of an AsyncTask?** - [Learn from here](https://stackoverflow.com/questions/18480206/asynctask-vs-thread-in-android)

* **什么是Loader？（已弃用） -从这里学习What is a `Loader`? (Deprecated)** - [Learn from here](https://developer.android.com/guide/components/loaders)

* **AsyncTaskan和 an的生命周期有什么关系Activity？这会导致什么问题？如何避免这些问题？What is the relationship between the life cycle of an `AsyncTask` and an `Activity`? What problems can this result in? How can these problems be avoided?**
    ...
    - 由于这些原因，将 AsyncTasks 用于长时间运行的后台任务通常不是一个好主意。相反，对于长时间运行的后台任务，应该采用不同的机制（例如服务）。
    ...
    - 注意：默认情况下，AsyncTasks 使用串行执行器在单个线程上运行，这意味着它只有 1 个线程，并且每个任务一个接一个地运行。

* **解释Looper,Handler和HandlerThread。Explain `Looper`, `Handler` and `HandlerThread`.**

* **Android 内存泄漏和垃圾收集Android Memory Leak and Garbage Collection**

#### Working With Multimedia Content 处理多媒体内容

* **你如何处理 Android 中的位图，因为它占用太多内存？-从这里和这里学习How do you handle bitmaps in Android as it takes too much memory?** - [Learn from here](https://developer.android.com/topic/performance/graphics/load-bitmap) and [here](https://developer.android.com/topic/performance/graphics/manage-memory)
...
* **Bitmap常规图像和九补丁图像有什么区别？What is the difference between a regular `Bitmap` and a nine-patch image?**
    ...
    - 通常，九补丁图像允许调整大小，可用作目标设备的背景或其他图像大小要求。九补丁指的是可以调整图像大小的方式：4 个角未缩放，4 个边缘在 1 个轴上缩放，中间一个可以缩放到两个轴。

* **说说Bitmap游泳池。-从这里学习Tell about the `Bitmap` pool.** - [Learn from here](https://amitshekhar.me/blog/bitmap-pool)
...
* **你将如何Activity在屏幕旋转期间保持状态？-从这里学习How would you preserve `Activity` state during a screen rotation?** - [Learn from here](https://stackoverflow.com/questions/3915952/how-to-save-state-during-orientation-change-in-android-if-the-state-is-made-of-m)
...
* **SharedPreferences 中的 commit() 和 apply() 是什么？What is commit() and apply() in SharedPreferences?**
    ...
    - apply() 是异步的，它不会返回任何布尔响应。如果您有未完成的 apply() 并且您正在执行 commit()，那么 commit() 将被阻止，直到 apply() 未完成。

#### Look and Feel 外观和感觉

* **什么是Spannable？What is a `Spannable`?**

* **什么是SpannableString？What is a `SpannableString`?**
...
* **在 Android 中使用文本的最佳做法是什么？What are the best practices for using text in Android?**
...
* **方法是什么onTrimMemory()？-从这里学习What is the `onTrimMemory()` method?** - [Learn from here](https://developer.android.com/topic/performance/memory)
...
* **什么是瞌睡？应用待机怎么样？-从这里学习What is Doze? What about App Standby?** - [Learn from here](https://developer.android.com/training/monitoring-device-state/doze-standby)

* **是什么overdraw？-从这里学习What is `overdraw`?** - [Learn from here](https://developer.android.com/topic/performance/rendering/overdraw.html)

#### Supporting Different Screen Sizes 支持不同的屏幕尺寸

* **您如何支持不同类型的决议？-从这里学习How do you support different types of resolutions?** - [Learn from here](https://developer.android.com/training/multiscreen/screensizes)
...
* **什么是 NDK，它为什么有用？ What is the NDK and why is it useful?** - Learn from here: [Android NDK and RenderScript](https://amitshekhar.me/blog/ndk-and-renderscript)

* **什么是渲染脚本？ What is renderscript?** - Learn from here: [Android NDK and RenderScript](https://amitshekhar.me/blog/ndk-and-renderscript)

#### Android System Internal 安卓系统内部

* **什么是安卓运行时？-安卓运行时 What is Android Runtime?** - [Android Runtime](https://amitshekhar.me/blog/dalvik-art-jit-aot)

* **Android 中的 Dalvik、ART、JIT 和 AOT - Dalvik、ART、JIT 和 AOT   Dalvik, ART, JIT, and AOT in Android** - [Dalvik, ART, JIT, and AOT](https://amitshekhar.me/blog/dalvik-art-jit-aot)

* **Dalvik 和 ART 有什么区别？- Dalvik 和 ART 的区别 What are the differences between Dalvik and ART?** - [Difference between Dalvik and ART](https://amitshekhar.me/blog/dalvik-art-jit-aot)

* **什么是去中心化交易所？-从这里学习 What is DEX?** - [Learn from here](https://developer.android.com/reference/dalvik/system/DexFile)

* **你能手动调用垃圾收集器吗？-是否可以在 Android 中强制垃圾收集？Can you manually call the Garbage collector?** - [Is it possible to force Garbage Collection in Android?](https://www.youtube.com/watch?v=fPEjpFKo1-Q)
...
* **Android 中的 LiveData 是什么？What is LiveData in Android?**
...
* **如何在 Android 中的 Fragment 之间共享 ViewModel？ How to share ViewModel between Fragments in Android?**
...
* **为什么要使用Bundle类来传递数据，为什么我们不能使用简单的Map数据结构？-从这里学习 Why Bundle class is used for data passing and why cannot we use simple Map data structure?** - [Learn from here](https://developer.android.com/guide/components/activities/parcelables-and-bundles)

* **您如何对崩溃的应用程序进行故障排除？-从这里学习 How do you troubleshoot a crashing application?** - [Learn from here](https://developer.android.com/topic/performance/vitals/crash)
...
* **什么是AAPT？-从这里学习 What is AAPT?** - [Learn from here](https://developer.android.com/studio/command-line/aapt2)

* **定期更新屏幕的最佳方法是什么？-从这里学习 What is the best way to update the screen periodically?** - [Learn from here](https://stackoverflow.com/questions/5452394/best-way-to-perform-an-action-periodically-while-an-app-is-running-handler)
...
* **详解OkHttp拦截器——从这里学习Explain OkHttp Interceptor** - [Learn from here](https://amitshekhar.me/blog/okhttp-interceptor)

* **OkHttp - HTTP 缓存-从这里学习OkHttp - HTTP Caching** - [Learn from here](https://amitshekhar.me/blog/caching-with-okhttp-interceptor-and-retrofit)

* **告诉我一些关于 RxJava 的事情。Tell me something about RxJava.**

* **你将如何处理 RxJava 中的错误？How will you handle error in RxJava?**

* **FlatMap 与 Map Operator -从这里学习FlatMap Vs Map Operator** - [Learn from here](https://amitshekhar.me/blog/rxjava-map-vs-flatmap)

* **RxJava什么时候用Create运算符，什么时候用运算符？fromCallable- 从这里学习：RxJava Create and fromCallable Operator When to use `Create` operator and when to use `fromCallable` operator of RxJava?** - Learn from here: [RxJava Create and fromCallable Operator](https://amitshekhar.me/blog/rxjava-create-and-fromcallable-operator)

* **什么时候使用deferRxJava 的运算符？- 从这里学习：RxJava Defer Operator  When to use `defer` operator of RxJava?** - Learn from here: [RxJava Defer Operator](https://amitshekhar.me/blog/rxjava-defer-operator)

* **RxJava 中如何使用 Timer、Delay 和 Interval 运算符？-从这里学习 How are Timer, Delay, and Interval operators used in RxJava?** - [Learn from here](https://amitshekhar.me/blog/rxjava-interval-operator)

* **如何使用 RxJava 并行进行两个网络调用？- RxJava 邮编运营商 How to make two network calls in parallel using RxJava?** - [RxJava Zip Operator](https://amitshekhar.me/blog/rxjava-zip-operator)

* **说出 Concat 和 Merge 的区别。-这里和这里学习 Tell the difference between Concat and Merge.** - [Learn from here](https://amitshekhar.me/blog/rxjava-concat-operator) and [here](https://amitshekhar.me/blog/rxjava-merge-operator)

* **在 RxJava 中解释主题？-从这里学习 Explain Subject in RxJava?** - [Learn from here](https://amitshekhar.me/blog/rxjava-subject-publish-replay-behavior-async)

* **RxJava 中的 Observable 有哪些类型？- 从这里学习：Types Observables In RxJava What are the types of Observables in RxJava?** - Learn from here: [Types Of Observables In RxJava](https://amitshekhar.me/blog/types-of-observables-in-rxjava)

* **如何在您的应用程序中使用 RxJava 实现搜索功能？- 从这里学习：使用 RxJava 运算符进行即时搜索 How to implement search feature using RxJava in your application?** - Learn from here: [Instant Search Using RxJava Operators](https://amitshekhar.me/blog/instant-search-using-rxjava-operators)

* **使用 RxJava 运算符在 RecyclerView 中分页-从这里学习 Pagination In RecyclerView Using RxJava Operators** - [Learn from here](https://amitshekhar.me/blog/pagination-in-recyclerview-using-rxjava-operators)

* **Android 图片加载库 Glide 和 Fresco 是如何工作的？-从这里、这里和这里学习 How The Android Image Loading Library Glide and Fresco Works?** - [Learn from here](https://amitshekhar.me/blog/android-image-loading-library-optimize-memory-usage), [here](https://amitshekhar.me/blog/android-image-loading-library-use-bitmap-pool-for-responsive-ui) and [here](https://amitshekhar.me/blog/android-image-loading-library-solve-the-slow-loading-issue)
...
* **为什么我们在Android中使用像Dagger这样的依赖注入框架？Why do we use the Dependency Injection Framework like Dagger in Android?**

* **匕首如何工作？How does the Dagger work?**

* **Dagger 中的组件是什么？What is Component in Dagger?**

* **Dagger 中的模块是什么？What is Module in Dagger?**

* **自定义范围在 Dagger 中如何工作？How does the custom scope work in Dagger?**

* **什么时候在 RxJava 的 CompositeDisposable 上调用 dispose 和 clear？-从这里学习 When to call dispose and clear on CompositeDisposable in RxJava?** - [Learn from here](https://amitshekhar.me/blog/dispose-vs-clear-compositedisposable-rxjava)

* **什么是网络中的多部分请求？ What is Multipart Request in Networking?**

* **Kotlin 中的 Flow 是什么？-从这里学习 What is Flow in Kotlin?** - [Learn from here](https://amitshekhar.me/blog/flow-api-in-kotlin)
...
* **描述 MVVM。- MVVM架构 Describe MVVM.** - [MVVM Architecture](https://amitshekhar.me/blog/mvvm-architecture-android)

* **MVC 与 MVP 与 MVVM 架构。 MVC vs MVP vs MVVM architecture.**

* **什么是主持人？What is presenter?**
...
* **设计优步应用程序。-从这里学习 Design Uber App.** - [Learn from here](https://github.com/amitshekhariitbhu/ridesharing-uber-lyft-app)
...
* **设计文件下载器-从这里学习Design File Downloader** - [Learn from here](https://github.com/amitshekhariitbhu/PRDownloader)

* **设计图像加载库-从这里、这里和这里学习 Design an Image Loading Library** - [Learn from here](https://amitshekhar.me/blog/android-image-loading-library-optimize-memory-usage), [here](https://amitshekhar.me/blog/android-image-loading-library-use-bitmap-pool-for-responsive-ui) and [here](https://amitshekhar.me/blog/android-image-loading-library-solve-the-slow-loading-issue)

* **HTTP 请求与 HTTP 长轮询与 WebSockets -从博客和视频中学习 - HTTP 请求与 HTTP 长轮询与 WebSocket 与服务器发送事件 HTTP Request vs HTTP Long-Polling vs WebSockets** - [Learn from blog](https://amitshekhar.me/blog/http-request-long-polling-websocket-sse) and [Video - HTTP Request vs HTTP Long-Polling vs WebSocket vs Server-Sent Events](https://www.youtube.com/watch?v=8ksWRX4xV-s)

* **语音和视频通话如何工作？-从这里学习 How do Voice And Video Call Work?** - [Learn from here](https://amitshekhar.me/blog/voice-and-video-call)

### Android Unit Testing 安卓单元测试

Android Interview Questions:安卓面试题：

* **什么是浓缩咖啡？-从这里学习 What is Espresso?** - [Learn from here](https://developer.android.com/training/testing/ui-testing/espresso-testing.html)

* **什么是 Robolectric？-从这里学习 What is Robolectric?** - [Learn from here](http://robolectric.org/)

* **使用 Robolectric 有哪些缺点？-从这里学习 What are the disadvantages of using Robolectric?** - [Learn from here](https://stackoverflow.com/questions/18271474/robolectric-vs-android-test-framework)

* **什么是 UI-Automator？-从这里学习 What is UI-Automator?** - [Learn from here](https://developer.android.com/training/testing/ui-testing/uiautomator-testing.html)

* **解释单元测试。-从这里学习 Explain unit test.** - [Learn from here](https://developer.android.com/training/testing/unit-testing/local-unit-tests)

* **解释仪器化测试。-从这里学习 Explain instrumented test.** - [Learn from here](https://developer.android.com/training/testing/unit-testing/instrumented-unit-tests)

* **你做过单元测试或自动测试吗？ Have you done unit testing or automatic testing?**

* **为什么使用 Mockito？-从这里学习 Why Mockito is used?** - [Learn from here](http://site.mockito.org/)

* **描述 JUnit 测试。-从这里学习 Describe JUnit test.** - [Learn from here](https://en.wikipedia.org/wiki/JUnit)
...
* **什么是亚行？-从这里学习 What is ADB?** - [Learn from here](https://developer.android.com/studio/command-line/adb)

* **什么是 DDMS，您可以用它做什么？-从这里学习 What is DDMS and what can you do with it?** - [Learn from here](https://developer.android.com/studio/profile/monitor)

* **什么是严格模式？- 从这里学习：StrictMode What is the StrictMode?** - Learn from here: [StrictMode](https://amitshekhar.me/blog/strictmode-in-android-development)

* **林特是什么？它是干什么用的？ What is Lint? What is it used for?**
...
* **您可以访问您的 SQLite 数据库进行调试吗？-从这里学习Can you access your database of SQLite Database for debugging?** - [Learn from here](https://github.com/amitshekhariitbhu/Android-Debug-Database)

* **使用Proguard有哪些需要注意的地方？What are things that we need to take care while using Proguard?**

* **Android 中的 Multidex 是什么？What is Multidex in Android?**
...
* **什么是迷惑？它是干什么用的？缩小化呢？What is obfuscation? What is it used for? What about minification?**

* **如何在不更新应用程序的情况下更改应用程序中的某些参数？How to change some parameters in an app without app update?**

### Java and Kotlin Java 和科特林
...
* **构造函数和方法有什么区别？What is the difference between a constructor and a method?**
    ...
    - 构造函数语法 -Constructor Syntax -
        ...
        ```
    - 注意：在上面的语法中，构造函数的名称与类的名称相同，并且没有返回类型。Note:
...
* **方法重载和覆盖之间的区别。Difference between method overloading and overriding.**
  <p align="center">
  <img alt="重载和覆盖Overloading and Overriding" src="https://github.com/youngWM/android-interview-questions-CN/blob/master/assets/overloading-vs-overriding.png">
  </p>
...
* **您知道哪些访问修饰符？每个人做什么？What are the access modifiers you know? What does each one do?** <br>
    - Java 语言中有四种访问修饰符（从最严格到最宽松）：There are four access modifiers in Java language (from strictest to the most lenient):
        ...
        3. protected可用于变量、方法和构造函数，因此只允许访问与受保护成员的类位于同一包内的子类和类。`protected` can be used on *variables*, *methods* and *constructors* therefore allowing access only to subclasses and classes that are inside the same package as protected members' class.
        4.public修饰符广泛用于类、变量、构造函数和方法，以授予从任何地方的任何类和方法的访问权限。它不应该在任何地方使用，因为它意味着标记为public不敏感的数据并且不能用于损害程序。 `public` modifier is widely-used on *classes*, *variables*, *constructors* and *methods* to grant access from any class and method anywhere. It should not be used everywhere as it implies that data marked with `public` is not sensitive and can not be used to harm the program.
...
* **什么是多态？继承呢？What is Polymorphism? What about Inheritance?**
...
* **什么是设计模式？What are the design patterns?**
    - 创作模式 Creational patterns
        - 生成器Builder [Wikipedia](https://en.wikipedia.org/wiki/Builder_pattern?oldformat=true)
        - 工厂Factory [Wikipedia](https://en.wikipedia.org/wiki/Factory_method_pattern?oldformat=true)
        - 单例Singleton [Wikipedia](https://en.wikipedia.org/wiki/Singleton_pattern)
        - 单州Monostate [Wikipedia](http://wiki.c2.com/?MonostatePattern)
        - 流畅的界面模式Fluent Interface Pattern [Wikipedia](https://martinfowler.com/bliki/FluentInterface.html)
...
#### Collections and Generics集合和泛型

* **数组与数组列表——从这里学习Arrays Vs ArrayLists** - [Learn from here](https://stackoverflow.com/questions/32020000/what-is-the-difference-between-an-array-arraylist-and-a-list/32020072)

* **HashSet 与 TreeSet -从这里学习HashSet Vs TreeSet** - [Learn from here](https://stackoverflow.com/questions/25602382/java-hashset-vs-treeset-when-should-i-use-over-other)

* **HashMap 与 Set -从这里学习 HashMap Vs Set** - [Learn from here](https://stackoverflow.com/questions/2773824/difference-between-hashset-and-hashmap)
...
* **什么是 Java 优先级队列？- 在优先队列中，每个元素都有一定的优先级，所有元素都出现在一个队列中。这些操作是根据优先级执行的。What is Java PriorityQueue?**
...
* **说 aString是不可变的是什么意思？What does it means to say that a `String` is immutable?**
...
* **是什么String.intern()？何时以及为什么应该使用它？What is `String.intern()`? When and why should it be used?**
...
* **Java 中的强引用、软引用、弱引用和虚引用是什么？What are strong, soft, weak and phantom references in Java?**

#### Concurrency 并发

* **关键字是什么synchronized意思？What does the keyword `synchronized` mean?**

* **什么是ThreadPoolExecutor？- Android 中的 ThreadPoolExecutor What is a `ThreadPoolExecutor`?** - [ThreadPoolExecutor in Android](https://amitshekhar.me/blog/threadpoolexecutor-in-android)
...
* **为什么不在构造函数中调用抽象方法？-从这里学习 Why would you not call abstract method in constructor?** - [Learn from here](https://stackoverflow.com/questions/15327417/is-it-ok-to-call-abstract-method-from-constructor-in-java)

* **你什么时候会创建一个对象值final？ When would you make an object value `final`?**

* **final这些finally和关键字是什么finalize？ What are these `final`, `finally` and `finalize` keywords?**
...
* **static这个词在Java中是什么意思？What does the `static` word mean in Java?**
...
* **如何StringBuilder实现 a 来避免不可变字符串分配问题？-从这里学习How is a `StringBuilder` implemented to avoid the immutable string allocation problem?** - [Learn from here](https://stackoverflow.com/questions/54023816/how-is-a-stringbuilder-implemented-to-avoid-the-immutable-string-allocation-prob)
...
* **val和 和有什么不一样var？-从这里学习 What is the difference between `val` and `var`?** - [Learn from here](https://stackoverflow.com/questions/44200075/val-and-var-in-kotlin)

* **在 Kotlin 中使用有什么优势const？-视频和博客 What is the advantage of using `const` in Kotlin?** - [Video](https://www.youtube.com/watch?v=3G49ivVxfkU) and [Blog](https://amitshekhar.me/blog/const-in-kotlin)

* **什么是 Kotlin 中的 JvmStatic 注解？-视频和博客 What is a JvmStatic Annotation in Kotlin?** - [Video](https://www.youtube.com/watch?v=qBBbOhY_pv4) and [Blog](https://amitshekhar.me/blog/jvmstatic-annotation-in-kotlin)

* **什么是 Kotlin 中的 JvmField 注解？-视频和博客 What is a JvmField Annotation in Kotlin?** - [Video](https://www.youtube.com/watch?v=bx8OZcMbeUE) and [Blog](https://amitshekhar.me/blog/jvmfield-annotation-in-kotlin)

* **什么是 Kotlin 中的 JvmOverloads 注解？-视频和博客 What is a JvmOverloads Annotation in Kotlin?** - [Video](https://www.youtube.com/watch?v=fHGsBV9Za8M) and [Blog](https://amitshekhar.me/blog/jvmoverloads-annotation-in-kotlin)

* **如何确保nullKotlin 的安全性？ How to ensure `null` safety in Kotlin?**

* **什么时候使用lateintKotlin 中使用的关键字？-从这里学习 When to use `lateint` keyword used in Kotlin?** - [Learn from here](https://amitshekhar.me/blog/lateinit-vs-lazy-in-kotlin)

* **如何检查一个lateinit变量是否已经初始化？-从这里学习 How to check if a `lateinit` variable has been initialized?** - [Learn from here](https://amitshekhar.me/blog/lateinit-vs-lazy-in-kotlin)

* **如何在 Kotlin 中对变量进行延迟初始化？-从这里学习 How to do lazy initialization of variables in Kotlin?** - [Learn from here](https://amitshekhar.me/blog/lateinit-vs-lazy-in-kotlin)

* **companion objects科特林有什么？-从这里学习 What are `companion objects` in Kotlin?** - [Learn from here](https://amitshekhar.me/blog/companion-object-in-kotlin)

* **Kotlin 中 == 和 === 的区别-视频和博客 Difference between == and === in Kotlin** - [Video](https://www.youtube.com/watch?v=lJtgxT2OIgQ) and [Blog](https://amitshekhar.me/blog/structural-and-referential-equality-in-kotlin)

* **Kotlin 中的可见性修饰符是什么？ What are the visibility modifiers in Kotlin?**

* **Kotlin 中 Java 静态方法的等价物是什么？ What is the equivalent of Java static methods in Kotlin?**

* **Kotlin 中的数据类是什么？ What is a data class in Kotlin?**

* **如何在 Kotlin 中创建单例类？ How to create a Singleton class in Kotlin?**

* **open和publicKotlin之间有什么区别？-从这里学习 What is the difference between `open` and `public` in Kotlin?** - [Learn from here](https://amitshekhar.me/blog/open-keyword-in-kotlin)
...
* **initKotlin 中的块是什么？-视频和博客 What is an `init` block in Kotlin?** - [Video](https://www.youtube.com/watch?v=cb3jOFozJns) and [Blog](https://amitshekhar.me/blog/init-block-in-kotlin)
...
* **Kotlin 中的协程是什么？-从这里学习 What are Coroutines in Kotlin?** - [Learn from here](https://amitshekhar.me/blog/kotlin-coroutines)

* **什么是协程作用域？-从这里学习 What is Coroutine Scope?** - [Learn from here](https://amitshekhar.me/blog/kotlin-coroutines)

* **什么是协程上下文？-从这里学习 What is Coroutine Context?** - [Learn from here](https://amitshekhar.me/blog/kotlin-coroutines)

* **在 Kotlin 协程中启动与异步-从这里学习 Launch vs Async in Kotlin Coroutines** - [Learn from here](https://amitshekhar.me/blog/launch-vs-async-in-kotlin-coroutines)

* **Kotlin 中的内联函数是什么？-视频和博客 What is inline function in Kotlin?** - [Video](https://www.youtube.com/watch?v=GLLI8h67ryo) and [Blog](https://amitshekhar.me/blog/inline-function-in-kotlin)
...
* **Kotlin 中的高阶函数是什么？- 从这里学习：Kotlin 中的高阶函数和 Lambda      What are higher-order functions in Kotlin?** - Learn from here: [Higher-Order Functions and Lambdas in Kotlin](https://amitshekhar.me/blog/higher-order-functions-and-lambdas-in-kotlin)

* **什么是 Kotlin 中的 Lambda - 从这里学习：Kotlin 中的高阶函数和 Lambda What are Lambdas in Kotlin** - Learn from here: [Higher-Order Functions and Lambdas in Kotlin](https://amitshekhar.me/blog/higher-order-functions-and-lambdas-in-kotlin)
...
* lifecycleScope, viewModelScope, GlobalScope 生命周期范围、viewModelScope、GlobalScope
* suspendCoroutine, suspendCancellableCoroutine suspendCoroutine, suspendCancellableCoroutine
* coroutineScope, supervisorScope 协程作用域、主管作用域

您可以在这里学习这些主题：Master Kotlin Coroutines    You can learn these topics here: [Master Kotlin Coroutines](https://amitshekhar.me/blog/kotlin-coroutines)


### Kotlin Flow API 科特林流 API

Kotlin Flow API for Android Interview 你应该知道的主题： Topics you should know in Kotlin Flow API for Android Interview:

* Flow Builder, Operator, Collector Flow Builder、Operator、Collector
* flowOn, dispatchers flowOn，调度员
* filter、map、zip、flatMapConcat、retry、debounce、distinctUntilChanged、flatMapLatest 等运算符 Operators such as filter, map, zip, flatMapConcat, retry, debounce, distinctUntilChanged, flatMapLatest
* 码头运营商 Terminal operators
* 冷流与热流：冷流与热流 Cold Flow vs Hot Flow: [Cold Flow vs Hot Flow](https://amitshekhar.me/blog/cold-flow-vs-hot-flow)
...
* **描述 REST API 的工作原理。什么是休息？ Describe how REST APIs work. What is REST?**
...
* **安卓开发最佳实践。- 从这里学习：Android 开发最佳实践 Android Development Best Practices.** - Learn from here: [Android Development Best Practices](https://amitshekhar.me/blog/android-development-best-practices)

* **你试过科特林吗？ Have you tried Kotlin?**

* **在 Android 应用程序开发过程中，您应该持续衡量哪些指标？- 从这里学习：Android 应用程序性能指标 What are the metrics that you should measure continuously while android application development?** - Learn from here: [Android App Performance Metrics](https://amitshekhar.me/blog/android-app-performance-metrics)
...
* **Kotlin Multiplatform 如何工作？-从这里学习 How does the Kotlin Multiplatform work?** - [Learn from here](https://amitshekhar.me/blog/how-does-the-kotlin-multiplatform-work)
...
* **Android 开发人员在下次面试时应该了解这些数据结构-检查此处 Android Developer should know these Data Structures for Next Interview** - [Check here](https://amitshekhar.me/blog/android-developer-should-know-these-data-structures-for-next-interview)
...
您可以通过以下方式与我联系： You can connect with me on:
